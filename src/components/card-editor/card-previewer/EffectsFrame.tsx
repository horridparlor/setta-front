...
const EffectsFrame: React.FC<EffectsFrameProps> = ({
  cardData,
  scale,
  cards,
}) => {
  ...
  const getMaterialsReminder = () => {
    if (!cardData.materialsReminder.length || isContinuous(cardData)) {
      return '';
    }
  ...
  const getMaterialsText = () => {
    ...
    let materials = [
      replaceIfEmpty(
        normalizeName(
          cards.find(card => isCardId(card, cardData.primaryMaterialId))
        ),
        'Primary'
      ),
      replaceIfEmpty(
        normalizeName(
          cards.find(card => isCardId(card, cardData.secondaryMaterialId))
        ),
        'Secondary'
      ),
      normalizeName(
        cards.find(card => isCardId(card, cardData.tertiaryMaterialId))
      ) ?? '',
    ];
    ...
    const materialsText = materials
      ...
      .join(` ${getMaterialsSeparator()} `);
    return isExtraDeckCard(cardData)
      ? materialsPrefix + materialsText + `){/h}${getMaterialsReminder()}\n`
      : '';
  };
  const getCostText = () => {
  ...
  const getFlavourText = () => {
    ...
    return hasFlavourText(cardData)
      ? flavourPrefix +
          (flavourText.length ? flavourText : 'Flavour text...') +
          '{/i}' +
          '\n'
      : '';
  };
  const getEffectsText = () => {
    return (
      getMaterialsText() + getCostText() + getEffectText() + getFlavourText()
    );
  };
  const getLineHeight = () => {
  ...
  const getTopPadding = () => {
    const firstLineSize = isExtraDeckCard(cardData)
      ? getMaterialsSize()
      : getEffectsSize();
    return Math.max(
      0,
      0.25 +
        0.06 * (firstLineSize + (isExtraDeckCard(cardData) ? 2 : 1)) -
        (getEffectsSize() + 1) * 0.045 * getLineHeight()
    );
  };
  const getSpecialCountsAsText = (id: number): string => {
    const entry = Object.entries(SpecialCountsAsId).find(
      ([_, value]) => value === id
    );
    return (entry ? entry[0] : '').replaceAll(
      '$EFFECTS_SIZE',
      getEffectsSize().toString()
    );
  };
  const getCountsAs = () => {
    const usesSpecialCountsAs =
      cardData.countsAsId !== null && cardData.countsAsId < 0;
    const countsAsText = usesSpecialCountsAs
      ? getSpecialCountsAsText(cardData.countsAsId || 0)
      : (normalizeName(
          cards.find(card => isCardId(card, cardData.countsAsId))
        ) ?? '');
    const countsAsPrefix = usesSpecialCountsAs
      ? ''
      : `Counts as ${
          starsWithVowel(countsAsText) ? 'an' : 'a'
        } {bi=${getEffectsSize()}}`;
      ...
      <>
        ...
        {formatText(
          countsAsPrefix +
            countsAsText +
            `${
              usesSpecialCountsAs ? '' : '{/bi}'
            }${endsSentence(countsAsText) || usesSpecialCountsAs ? '' : '.'}`,
          scale
        )}
      </>
    );
  ...
  return (
    <Typography
      ...
      sx={{
        ...
        opacity: isDeckMaster(cardData) ? 0 : 1,
        borderRadius: '0.4rem',
