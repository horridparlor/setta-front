...
const CardFilters: React.FC<CardFiltersProps> = forwardRef<
  CardFiltersRef,
  CardFiltersProps
>(
  (
    ...
    const cookieParams =
      sessionStorage.getItem(PageCookie.CARD_CATALOGUE_FILTERS) || '';
    const params = urlParams.size
      ? urlParams
      : new URLSearchParams(cookieParams);
    ...
    useEffect(() => {
      onFilterChange({
        ...
        defOperation,
        isAce:
          typeof isAce === 'boolean' ? isAce : isAce.toLowerCase() === 'true',
        ownerId,
        isReleased:
          typeof isReleased === 'boolean'
            ? isReleased
            : isReleased.toLowerCase() === 'true',
        isErrata:
          typeof isErrata === 'boolean'
            ? isErrata
            : isErrata.toLowerCase() === 'true',
      });
    }, [
      cardName,
      cardEffects,
    ...
    const handleLevelOperationChange = (
      _event: React.MouseEvent<HTMLElement>,
      value: string | null
    ) => {
      if (value === null) {
    ...
    const handleAtkOperationChange = (
      _event: React.MouseEvent<HTMLElement>,
      value: string | null
    ) => {
      if (value === null) {
    ...
    const handleDefOperationChange = (
      _event: React.MouseEvent<HTMLElement>,
      value: string | null
    ) => {
      if (value === null) {
      ...
      <StyledAppBar position="sticky" sx={{ margin: '0.2rem 0' }}>
        <Toolbar disableGutters>
          ...
          <FiltersModal
            ...
            cardClass={cardClass}
            handleCardClassChange={handleCardClassChange}
            cardType={cardType}
            handleCardTypeChange={handleCardTypeChange}
            cardSubtype={cardSubtype}
            handleCardSubtypeChange={handleCardSubtypeChange}
            cardSupertype={cardSupertype}
            handleCardSupertypeChange={handleCardSupertypeChange}
            cardDeck={cardDeck}
            ...
            handleLevelOperationChange={handleLevelOperationChange}
            atk={atk as number}
            handleAtkChange={handleAtkChange}
            atkOperation={atkOperation}
            handleAtkOperationChange={handleAtkOperationChange}
            def={def as number}
            handleDefChange={handleDefChange}
            defOperation={defOperation}
            handleDefOperationChange={handleDefOperationChange}
            isAce={isAce === true || isAce === 'true'}
            handleIsAceChange={handleIsAceChange}
            ownerId={ownerId}
            handleOwnerIdChange={handleOwnerIdChange}
            isReleased={isReleased === true || isReleased === 'true'}
            handleIsReleasedChange={handleIsReleasedChange}
            isErrata={isErrata === true || isErrata === 'true'}
            handleIsErrataChange={handleIsErrataChange}
