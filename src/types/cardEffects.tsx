...
const TARGET_OR_ALL_TARGET_TYPE_OPTIONS = [TargetType.ALL, TargetType.TARGET];
...
export const isEffectsDirection = (
  value: string
): value is EffectsDirection => {
...
export const isEffectsHindrance = (
  value: string
): value is EffectsHindrance => {
  ...
  switch (cardData.cardEffects.cost.costType) {
    case CostType.PAYMENT:
      return Object.values(PaymentCostType).filter(
        type => type !== PaymentCostType.NONE
      );
    case CostType.STATE:
...
export const getDefaultCostSubtype = (cardData: CardData) => {
  switch (cardData.cardEffects.cost.costType) {
    ...
    case CostType.STATE:
      return isMonster(cardData)
        ? StateCostType.SUMMONED_THIS_TURN
        : StateCostType.COUNT_CARDS;
    case CostType.TRIGGER:
      return isTrap(cardData)
        ? TriggerCostType.OPPONENT
        : TriggerCostType.SELF_TRIGGERED;
    default:
      return '';
...
export const fixSelectionTypeByTarget = (
...
const routeCostSelectionType = (cost: EffectsCost) => {
  ...
  switch (cost.costType) {
    case CostType.PAYMENT:
      selectionType = routePaymentCostTypeSelectionType(
        cost.subtype || PaymentCostType.NONE
      );
...
export const getCostPrestateOptions = (cardData: CardData) => {
  const options = Object.values(StateCostType).filter(
    type => ![StateCostType.COUNT_CARDS].includes(type)
  );
  const cost = getEffectsCost(cardData);
  switch (cost.costType) {
    case CostType.PAYMENT:
...
export function isCountedAmount(
  amount: number | EffectsCount | null
): amount is EffectsCount {
...
export const getEffectTargetTypeOptions = (cardData: CardData) => {
  ...
  switch (effect.effectType) {
    ...
    case EffectType.STAT:
      ...
      return isMonster(cardData)
        ? THIS_TARGET_OR_ALL_TARGET_TYPE_OPTIONS
...
const routeTriggerTargetOptions = (cardData: CardData) => {
  const cost = getEffectsCost(cardData);
  if (
    cost.costType === CostType.TRIGGER &&
...
const routeDefaultTriggerTarget = (cardData: CardData) => {
  ...
  if (
    ...
    switch (cost.supertype) {
      case OpponentTriggerCostType.ATTACKS:
        return options.includes(TargetType.ATTACKER)
          ? TargetType.ATTACKER
          : TargetType.TARGET;
      case OpponentTriggerCostType.SUMMONS:
        return options.includes(TargetType.ATTACKER)
...
export const getDefaultEffectTargetType = (cardData: CardData) => {
  ...
  switch (effect.effectType) {
    ...
    case EffectType.STAT:
      ...
      return isMonster(cardData)
        ? TargetType.THIS
        : routeDefaultTriggerTarget(cardData);
  }
  return TargetType.NONE;
};
  ...
  switch (effect.effectType) {
    ...
    case EffectType.STAT:
      switch (effect.subtype) {
        ...
        case StatEffectType.LIFE:
          if (
            TRIGGERED_TARGETING_TARGET_TYPES.includes(
              effect.target?.targetType || TargetType.NONE
            )
          ) {
