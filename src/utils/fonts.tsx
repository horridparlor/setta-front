...
const createStyledSpan = (styleProps: {
  fontWeight: string;
  fontStyle?: string;
}) =>
  styled('span')<{ fontSize?: number; scale: number }>(props => ({
    fontWeight: styleProps.fontWeight,
    fontStyle: styleProps.fontStyle,
    fontSize: props.fontSize
      ? getFontSize(props.fontSize, props.scale)
      : undefined,
  }));

export function addSizeToRichText(
...
const LightItalic = createStyledSpan({
  fontWeight: '300',
  fontStyle: 'italic',
});
...
export const formatText = (text: string, scale: number): JSX.Element[] => {
  ...
  text.replace(regex, (match, tag, fontSize, content, _, offset) => {
    ...
    switch (tag) {
      ...
      case 'sb':
        result.push(
          <SemiBoldNormal {...elementProps}>{content}</SemiBoldNormal>
        );
        break;
      case 'r':
      ...
      case 'bs':
        result.push(
          <BoldItalic sx={{ textDecoration: 'line-through' }} {...elementProps}>
            {content}
          </BoldItalic>
        );
        break;
