...
export const encodeEffectsString = (message: string) => {
    const tagPattern = /(\{[isb]*\}.*?\{\/[isb]*\})|(\b(normal|revenge|royal|fusion|ritual|time traveller|pendulum|hand trap|maximum|defense|attack|face-up|face-down|piercing|chain-attack|triple-attack|sleeptalk|armor-up|goaded|ruler|except|\d+|(?<=or|and\s+)lower|(?<=or|and\s+)higher|(?<=you|opponent\s+)may|(?<=If\s+)set|(?<=copies of the\s+)same|this(?=\s+turn)|cannot(?=\s+be)|don't(?=\s+have))\b)|(\b(Spell|Spells|Trap|Traps|atk|def|Abyss|Dragon|Dragons|Kawaii|Slime|Slimes|Sparks|Zombie|Zombies)\b)/g;
    return message
        .replace('(', '{i}')
        .replace('.)', ')')
        .replace(')', '{/i}')
